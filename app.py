...
:root {
  ...
  --bg1: #070d1a;
  --bg2: #0b1425;
  --bg3: rgba(11,20,37,0.7);
  --border: rgba(0,255,225,0.12);
  --border2: rgba(0,144,255,0.15);
  --hi: #ddeeff;
  --lo: rgba(180,210,240,0.45);
}
...
.stApp {
  background:
    radial-gradient(ellipse 80% 40% at 15% 10%, rgba(0,90,255,0.07) 0%, transparent 60%),
    radial-gradient(ellipse 60% 30% at 85% 90%, rgba(0,255,200,0.05) 0%, transparent 60%),
    var(--bg0) !important;
}
...
.stTabs [aria-selected="true"] {
  ...
  box-shadow: 0 0 15px rgba(0,255,225,0.15) !important;
}
...
.nx-card::before {
  ...
  background: linear-gradient(90deg, transparent 0%, var(--c) 40%, var(--c2) 70%, transparent 100%);
  opacity: 0.5;
}
.nx-card.red { border-color: rgba(255,48,96,0.25); }
.nx-card.red::before { background: linear-gradient(90deg, transparent, var(--red), transparent); }
.nx-card.amber { border-color: rgba(255,170,0,0.25); }
.nx-card.amber::before { background: linear-gradient(90deg, transparent, var(--amber), transparent); }
...
.kpi-lbl {
  ...
  text-transform: uppercase; color: var(--lo); margin-bottom: 5px;
}
.kpi-val {
...
.kpi-val.red { background: linear-gradient(135deg, #ff3060, #ff6090); -webkit-background-clip: text; background-clip: text; }
.kpi-val.amber { background: linear-gradient(135deg, #ffaa00, #ffcc55); -webkit-background-clip: text; background-clip: text; }
...
.ticker-outer::before { left:0; background: linear-gradient(90deg, var(--bg0), transparent); }
.ticker-outer::after  { right:0; background: linear-gradient(-90deg, var(--bg0), transparent); }
.ticker-t {
  display: inline-block;
  animation: scr 40s linear infinite;
  font-family: 'JetBrains Mono', monospace;
  font-size: 0.72rem; color: var(--c); letter-spacing: 1px;
  padding-left: 100%;
}
...
.b-go   { background:rgba(0,255,136,0.12); color:var(--green); border:1px solid rgba(0,255,136,0.35); }
.b-slow { background:rgba(255,170,0,0.12);  color:var(--amber); border:1px solid rgba(255,170,0,0.35); }
.b-stop { background:rgba(255,48,96,0.12);  color:var(--red);   border:1px solid rgba(255,48,96,0.35); }
...
.alert-crit { background:rgba(255,48,96,0.1); border:1px solid rgba(255,48,96,0.4); }
...
.ai-msg {
  ...
  margin-bottom:10px; font-size:.82rem; line-height:1.6;
}
.user-msg {
  background:rgba(0,255,225,0.06); border:1px solid var(--border);
...
with tabs[4]:
    ...
    with ai1:
        ...
        for qp in quick_prompts:
            st.markdown(f'<span class="b b-info" style="cursor:pointer;font-size:.65rem;">{qp}</span>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
- Active Incidents: {len(traffic['incidents'])} — {', '.join([i['type']+' on '+i['road'] for i in traffic['incidents'][:3]])}
- Weather: {weather_desc}, {temp:.0f}°C, Wind {wind_speed:.0f} km/h, Precip {precip:.1f} mm/hr
- Air Quality: AQI {aqi_val}, PM2.5 {pm25:.1f} µg/m³
- Visibility: {visibility/1000:.1f} km
- Time: {datetime.now().strftime('%H:%M')}, {'Weekend' if datetime.now().weekday()>=5 else 'Weekday'}
- Most congested intersections: {', '.join([i['name']+' ('+str(i['congestion'])+'%)' for i in sorted(traffic['intersections'],key=lambda x:-x['congestion'])[:3]])}

Be concise, data-driven, and professional. Use bullet points. Reference specific roads and numbers."""
